...
class ComparisonPlot:
    ...
    def get_overlay_text(self, generation: int):
        return self.base_text + \
               f"Best fitness: {self.statistics['best_fitness'][generation]}\n" \
               f"Average fitness: {self.statistics['average_fitness'][generation]}\n" \
               f"Selection pressure: {self.statistics['selection_pressure'][generation]}\n" \
    ...
    def __init__(self, evolution_data, statistics_file: str, title: str, *baseline_data, **kwargs):
        ...
        self.base_text = f"Population Size: {len(self.evolution_data.population(0))}\n" \
                         f"Maximum generations: {len(self.evolution_data)}\n"
        if "extra_text" in kwargs:
            self.base_text += kwargs["extra_text"]

        self.specification_text = self.figure.text(0.05, 0.8, self.get_overlay_text(0), size=8)
        ...
        self.best_fitness_line = self.init_statistics_line(self.axis_2d_bottom, "best_fitness", "Best Fitness", 1,
                                                           "black")
        self.axis_2d_bottom.legend(loc='upper right')
    ...
    def init_axis_2d_bottom(self, **kwargs):
        ...
        axis.set_yscale('log')
        axis.set_ylim(1E-12, self.statistics["average_fitness"].max())
        twin = axis.twinx()
        twin.set_ylim(0., 1.)
    ...
    def init_statistics_line(self, axis, channel: str, description: str, length: int, color: str):
        line, = axis.plot(self.get_statistics_data(channel, length), label=description, c=color)
        return line
    ...
    def update(self, frame: int):
        ...
        self.specification_text.set_text(self.get_overlay_text(frame))
...
if __name__ == "__main__":
    ...
    plot = ComparisonPlot(
        ...
        ylim=(-5.12, 5.12),
        extra_text="Selection for Reproduction: Tournament (Tournament Size = 5, Selection Size = 10)\n"
                   "Selection for Replacement: Truncation (Selection Size = 50\n"
