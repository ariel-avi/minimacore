...
protected:
  ...
  void test_ranked_selection_for_replacement_by_ranks(size_t rank_count) {
    ...
    std::ranges::for_each(test_set, [&](auto &individual) {
      EXPECT_TRUE(std::find(_population.begin(), _population.end(), individual) == _population.end())
...
TYPED_TEST(minimacore_genetic_algorithm_tests, uniform_mutation) {
  vector<TypeParam> factors{{1., 2., 3., 4., 5., 6.}};
...
TYPED_TEST(minimacore_genetic_algorithm_tests, evolution_statistics) {
  ...
  {
    auto best_fitness_stat =
        this->_statistics[static_cast<int>(statistics_requests_factory<TypeParam>::stat_requests::best_fitness_stat)];
...
TYPED_TEST(minimacore_genetic_algorithm_tests, selection_pressure_termination_positive) {
  ...
  ASSERT_TRUE(termination(this->_statistics))
      ...
      << this->_statistics.current_value(
             static_cast<int>(statistics_requests_factory<TypeParam>::stat_requests::selection_pressure_stat));
}
...
public:
  ...
  explicit population_initialization_fail_mock(size_t max_failures) : _max_failures(max_failures) {}
};
